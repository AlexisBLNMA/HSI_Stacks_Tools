...
def interactive1(dc, g, s, Ro, nbit, histeq=True, ncomp=5, filt=False, nfilt=0, spectrums=False,
    ...
    ccolor = ['green', 'red', 'blue', 'yellow', 'darkviolet']
    for i in range(ncomp):
            circle = plt.Circle((center[i][0], center[i][1]), Ro * factor, color=ccolor[i], fill=False, linewidth= CW)
    ...
    if spectrums:
        ...
        for i in range(ncomp):
            if lamd.any():
                #plt.plot(lamd, spect[i], ccolor[i])      Can be deleted
                min_len = min(len(lamd), len(spect[i]))   #New line
